  ...
  return (
    <>
      ...
      <ul className='list-none flex justify-center flex-col items-center'>
        {todos.map((todo) => (
          <li
            className='w-1/2 mt-4 flex justify-between items-center bg-zinc-800 px-4 py-2 rounded'
            key={todo.id}
          >
            <input
              type='checkbox'
              className='h-6 w-6 
            border-2 border-gray-300 rounded cursor-pointer
            '
            />
            <div className='text-white w-full'>{todo.title}</div>
            <div className='w-1/3 flex justify-between'>
              <button className='text-white bg-green-500 border-0 py-1 px-4 mr-2 focus:outline-none hover:bg-green-600 rounded text-md'>
                <svg
